...
def showcap():
    ...
    try:
        img = PhotoImage(file='C:\\Users\\rishi\\Desktop\\projectpython\\captchaone.png')  # Update with the correct path
...
def showdash():
    ...
    tm = Toplevel(t)
    tm.geometry("1000x800")  # Adjusted for full screen within a reasonable size
...
# Main window setup
t = Tk()
t.geometry('800x800')  # Set an initial window size
t.config(bg='#1E3A3A')  # Set the background color of the login screen

# Login screen components
bt1 = Button(t, text="Generate Captcha", command=gen, bg="yellow", font=("Arial", 12, "bold"))
bt1.place(x=50, y=50)

bt2 = Button(t, text="Show Captcha", command=showcap, bg="yellow", font=("Arial", 12, "bold"))
...
e = Label(t, text="Password", bg='#1E3A3A', fg="white", font=("Arial", 12))
e.place(x=50, y=450)

f = Entry(t, width=20, show="*")  # Hide the password input
f.place(x=150, y=450)

g = Label(t, text="Captcha", bg='#1E3A3A', fg="white", font=("Arial", 12))
g.place(x=50, y=500)

h = Entry(t, width=20)
h.place(x=150, y=500)

bt3 = Button(t, text="Login", command=login, bg="green", fg="white", font=("Arial", 14, "bold"))
bt3.place(x=100, y=560)

bt4 = Button(t, text="Close", command=cts, bg="red", fg="white", font=("Arial", 14, "bold"))
bt4.place(x=200, y=560)
